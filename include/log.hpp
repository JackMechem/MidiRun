...
#define LOG_WARNING(...)                                                       \
	Log(std::clog, "WARNING", __FUNCTION__, __LINE__).write(__VA_ARGS__)
#define LOG_ERROR(...)                                                         \
	Log(std::clog, "ERROR", __FUNCTION__, __LINE__).write(__VA_ARGS__)
#define LOG_DEBUG(...)                                                         \
	Log(std::clog, "DEBUG", __FUNCTION__, __LINE__).write(__VA_ARGS__)

inline std::string
getLogTime(std::chrono::time_point<std::chrono::system_clock> time) {
	auto epoch_seconds = std::chrono::system_clock::to_time_t(time);
	std::stringstream stream;
	stream << std::put_time(gmtime(&epoch_seconds), "[%F_%T");
	auto truncated = std::chrono::system_clock::from_time_t(epoch_seconds);
	auto delta_us =
		std::chrono::duration_cast<std::chrono::milliseconds>(time - truncated)
			.count();
...
template <typename TF> void writeLog(std::ostream &out, TF const &f) {
	out << f;
}
...
struct Log {
	...
	template <typename TF> void write(TF const &f) { writeLog(out, f); }
